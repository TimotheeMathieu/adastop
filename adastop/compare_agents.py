...
class MultipleAgentsComparator:
        ...
        if k == 0:
            for i, comp in enumerate(comparisons):
                ...
                if self.B > n_permutations:
                    ...
                    permutations = [np.arange(n1)] + [
                        self.rng.permutation(n1 + n2)[:n1] for _ in range(self.B - 1)
                    ]
                    self.normalization = self.B

                # Compute the mean differences on the evaluations for each comparisions and for each permutation
                mean_diff = []
            ...
            for i, comp in enumerate(comparisons):
                ...
                # Can be either all the permutations of block k, or using random permutations if this is too many.
                n_permutations = math.comb(n1 + n2, n1)
                if self.B > n_permutations ** (k + 1):
                    ...
                    permutations_k = [np.arange(n1)] + [
                        self.rng.permutation(n1 + n2)[:n1]
                        for _ in range(n_perm_to_add - 1)
                    ]
                    began_random_at = np.floor(np.log(self.B) / np.log(n_permutations))
                    self.normalization = (
                        n_permutations**began_random_at
                    )  # number of permutations
                ...
                for id_p, perm_k in enumerate(permutations_k):
                    if self.B > n_permutations ** (k + 1):
                        perms_before_k = np.arange(
                            len(mean_diffs[str(comparisons[0])])
                        ).astype(int)
                    else:
                        perms_before_k = [id_p]
                    ...
                    for perm_before_k in perms_before_k:
                        ...
                        mask[list(perm_k)] = 1
                        mask = mask == 1
                        new_mean_diffs.append(
                            mean_diffs[str(comp)][perm_before_k]
                            + np.mean(Zk[mask])
                            - np.mean(Zk[~mask])
                        )
    ...
    def partial_compare(self, eval_values, verbose=True):
        ...
        if self.k == 0:
            ...
            self.id_tracked = np.arange(
                len(self.decisions)
            )  # ids of comparisons effectively tracked
        ...
        for j in range(len(current_decisions)):
            current_comparisons = self.current_comparisons[
                current_decisions == "continue"
            ]
            mean_diffs_now = {
                str(comp): mean_diffs[str(comp)] for comp in current_comparisons
            }
            max_mean_diffs = [
                np.max([mean_diffs_now[str(comp)][i] for comp in current_comparisons])
                for i in range(len(mean_diffs_now[str(current_comparisons[0])]))
            ]
            values = np.sort(max_mean_diffs)

            icumulative_probas = (
                np.arange(len(values))[::-1] / self.normalization
            ...
            # Test statistic, step-down multiple comparison method
            Tmax = 0
            Tmin = np.inf
            Tmaxsigned = 0
            Tminsigned = 0
            for i, comp in enumerate(
                self.current_comparisons[current_decisions == "continue"]
            ):
                Ti = (k + 1) * np.abs(
                    np.mean(Z[comp[0]][: ((k + 1) * self.n[comp[0]])])
            ...
            if Tmax > bk_sup:
                id_reject = np.arange(len(current_decisions))[
                    current_decisions == "continue"
                ][imax]
        ...
        self.mean_diffs = {
            str(comp): self.mean_diffs[str(comp)] for comp in current_comparisons
        }
                    ...
                    pd.DataFrame(
                        {
                            "Agent1 vs Agent2": [
                                "{0} vs {1}".format(
                                    self.agent_names[c[0]], self.agent_names[c[1]]
                                )
                            ],
                            "mean Agent1": self.mean_eval_values[c[0]],
                            "mean Agent2": self.mean_eval_values[c[1]],
                            "mean diff": self.mean_eval_values[c[0]]
                            - self.mean_eval_values[c[1]],
                            "std Agent 1": np.std(
                                self.eval_values[self.agent_names[c[0]]]
                            ),
                            "std Agent 2": np.std(
                                self.eval_values[self.agent_names[c[1]]]
                            ),
